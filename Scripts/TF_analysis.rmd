...
# for (i in 1:length(mm_genes)) {
#     s <- as.data.frame(mm_genes[i])
#     print(length(rownames(s)))
# }

for (gene_name in 1:length(unique(mm_genes[[6]]$gene_symbol))) {
...
mean(width(all_proteins))

writeXStringSet(AAStringSet(all_proteins),
                filepath = paste0(RESULTS, "Extracted_protein_sequences_2.fasta"),
                format = "fasta")
...
predictions <-
    predict(human_db, all_proteins,
            ...
            custom_format = "qseqid sseqid qacc sacc length pident ppos qcovs evalue")

# Saving blastp result into a file so that it can be used for further analysis:
# FIND BETTER WAY TO SAVE THE RESULT!
sink(paste0(PROJECT, "predictions_05-18.txt"))
print(predictions)
sink()
# nolint end
```
