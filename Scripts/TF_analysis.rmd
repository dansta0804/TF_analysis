...
org.Hs.eg.db, annotables, rBLAST, rentrez, ChIPpeakAnno)
# nolint end
```
...
gene_sequencess <- gene_sequences[100:600]
names(gene_sequences[100:200])
predictions <-
    predict(human_db, gene_sequencess, BLAST_args = "-perc_identity 99",
...
for (acc in 1:length(predictions$qseqid)) {
    ...
    #                           rettype = "fasta")
    # write(sequence, paste0(PROJECT, predictions$sacc[acc]))
...
# significant peaks:
significant_peaks <- grl_hg[[1]][grl_hg[[1]]$q_value < 2, ]





subsetByOverlaps(mm_genes[[1]], significant_peaks)
