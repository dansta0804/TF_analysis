...
org.Hs.eg.db, annotables, rBLAST)
```
...
DATABASE    <- paste0(PROJECT, "Input/Genome_DB/human_genome.00")
# nolint end
```
...
gene_sequences <- getSeq(BSgenome.Mmusculus.UCSC.mm10, mm_genes)

for (sample in 1:length(gene_sequences)) {
    for (gene in 1:length(gene_sequences[[sample]])) {
        names(gene_sequences[[sample]][gene]) <-
            mm_genes[[sample]][gene]$gene_symbol
    }
}
# nolint end
```
