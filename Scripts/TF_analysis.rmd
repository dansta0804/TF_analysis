...
for (sample in seq_along(mm_annotated_peaks)) {
    selected_genes <-
        ...
        # length(unique(mm_annotated_peaks[[sample]]$gene_symbol))
        # length(mmgene_ranges$gene_symbol)
                            
    genes <- mmgene_ranges[selected_genes]
    # genes <- mm_annotated_peaks[[sample]][selected_genes]
...
writeXStringSet(AAStringSet(all_proteins), filepath = paste0(PROJECT, "Rez.fasta"), format = "fasta")
...
system(paste0("blastp -query ", paste0(PROJECT, "Rez.fasta"), " -db ", human_db, " -out mm-second.x.zebrafish.txt -num_threads 10"))
