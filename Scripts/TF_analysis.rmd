...
predictions <- predictions[predictions$`Perc.Ident` > 60, ]
...
for (id in 1:length(matched_data$EntrezQuery)) {
    seq_m <- getSeq(BSgenome.Mmusculus.UCSC.mm10,
                       mmgene_ranges[mmgene_ranges$gene_id == matched_data$EntrezQuery[id], ])
    names(seq_m) <- matched_data[id, "QuerySymbol"]
    mm_sequences <- append(mm_sequences, seq_m)

    seq_h <- getSeq(BSgenome.Hsapiens.UCSC.hg38, hggene_ranges[hggene_ranges$gene_id == matched_data$EntrezSubject[id], ])
...
countPWM(as.matrix(pwm), as.character(mm_sequences[[14]]), min.score = "65%")
countPWM(as.matrix(pwm), as.character(hg_sequences[[14]]), min.score = "65%")
# nolint end
```
