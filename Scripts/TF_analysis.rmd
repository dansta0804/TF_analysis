...
for (sample in seq_along(mm_annot_peaks)) {
    selected_genes <-
        tolower(mm_annot_peaks[[6]]$gene_symbol) %in%
    ...
    mm_genes[[4]] <-
        mmgene_ranges[mmgene_ranges$gene_symbol %in% mm_annot_peaks[[6]][selected_genes]$gene_symbol]
...
for (file in 1:length(mm_genes)) {
    ...
    for (gene_name in 1:length(unique(mm_genes[[4]]$gene_symbol))) {
        gene <- mm_genes[[4]]$gene_symbol[[gene_name]]
        if (is.na(gene)) {
            next
        } else {
            proteins <-
    ...
    write.csv(pre, paste0(RESULTS, "Blastp/predictions_sample4_full.csv"),
              row.names = FALSE)
}

all_proteins <-
    readAAStringSet(paste0(INT_FILES,
                           "Extracted_protein_sequences_sample4.fasta"),
                    format="fasta")
# nolint end
```
...
find_PWM_hits <- function (mm_seq_list, hg_seq_list, pwm, min_score) {
    for (seq in 1:length(mm_seq_list)) {
    # nolint start
    ...
    for (gene in 1:length(mm_seq_list[[seq]])) {
        mm_hits <-
            as.numeric(countPWM(as.matrix(pwm), as.character(mm_seq_list[[seq]][gene]),
                                min.score = min_score))

        row <- c(names(mm_seq_list[[seq]][gene]), mm_hits)
        percentages_query[nrow(percentages_query) + 1, ] <- row            
    }

    print("oook")
    write.csv(percentages_query, paste0(RESULTS, paste0("PWM_hits/percent_mm_", seq), ".csv"),
            row.names = FALSE)
    percentages_query$Gene <- toupper(percentages_query$Gene)
print("osssss")
    for (gene in 1:length(hg_seq_list[[seq]])) {
        hg_hits <- as.numeric(countPWM(as.matrix(pwm),
                                    as.character(hg_seq_list[[seq]][gene]),
                                    min.score = min_score))

        row <- c(names(hg_seq_list[[seq]][gene]), hg_hits)
        percentages_subject[nrow(percentages_subject) + 1, ] <- row            
    }

    write.csv(percentages_subject, paste0(RESULTS, paste0("PWM_hits/percent_hg_", seq), ".csv"),
            row.names = FALSE)
...
results <- list("df" = df, "merges" = percent_merges)
return(results)
}
