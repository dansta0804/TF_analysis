...
server <- function(input, output, session) {
       ...
       # Padaryti, kad būtų automatiškai sukuriami nauji pavadinimai mėginiams,
       # kurie bus panaudoti grafikuose.

       output$samples <-
              DT::renderDataTable({input$bigbed[, c("name", "size")]},
              server = TRUE)
...
# GENOMIC DATA ANALYSIS - TF MOTIF (PLOT 7):
              ...
              output$table1 <- DT::renderDataTable({
                  ...
                  system(paste("findMotifsGenome.pl", paste0(INPUTS, names), "mm10",
                      paste0(PROJECT, RESULTS, names, "/"), "-size 200 -mask"), intern = FALSE)

                  homer_motifs <- read.table(paste0(PROJECT, RESULTS, names, "/knownResults.txt"),
