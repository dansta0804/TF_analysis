...
server <- function(input, output, session) {
  ...
  observe({
    ...
    output$plot15 <- renderPlot({
      ...
      # GALI BŪTI KLAIDA SU PAVADINIMAIS!

      ggplot(percentages$df, aes(x = Sample, y = as.numeric(Percentage))) +
      geom_bar(width = 0.4, color = "black", fill = "#930d1f",
               stat = "identity") +
      labs(title = paste0("Min.score = ", input$min_score), x = "",
           y = "", size = 5) +
      geom_text(aes(label = paste0(round(as.numeric(Percentage), 2), "%")),
